...
public:
	...
	public:
		reference operator*() {
			...
			size_t index = (parent->begin_index + offset) % MAX_SIZE;
...
public:
	...
	ItemType front() const {
		...
		{
			throw std::exception("Warning: Empty ring!");
		}
	...
	//	if (ring_size == size)
	//	{
	//		size_t temp = (begin_index++) % size;
	//		begin_index = temp;
	//		end_index = temp;
	//	}
	//	else if (ring_size < size)
	//	{
	...
	void pop_front() {
		if (ring_size == 0)
		{
			throw std::exception("Size is zero!");
		}
		else 
		{
			--ring_size;
			begin_index = (begin_index < MAX_SIZE-1 ) ? begin_index + 1 : 0;	//TURNARY OPERATOR
		}
